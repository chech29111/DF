...
//         //console.log('dataItemIdx',dataItemIdx)
//         if (dataItemIdx === -1) {
//             const groupItem = parseDivisionItem(dataItem);
//             //console.log('groupItem',groupItem)
//             groupedData.push(groupItem);
//         }
//         else {
//             const itemDivisions = parseDivisionItem(dataItem);
//             // console.log('itemDivisions',itemDivisions)
//             groupedData[dataItemIdx].works.push(itemDivisions.works[0]);
//         }
//
//     });
//
...
//
//
//         const dataItemIdxUp = groupedType.findIndex((grpItem) =>  grpItem.id === dataItem.fldUpDivisionId);
//
//         if (dataItemIdxUp === -1) {
//             const groupItem = parseDivisionItem(dataItem);
//             //console.log('groupItem',groupItem)
//             groupedType.push(groupItem);
//         }
//         else {
//              const itemDivisions = parseDivisionItem(dataItem);
//             //  console.log('itemDivisions.divisions[0].works',itemDivisions.divisions[0].works)
//             // for (let i=0;i<groupedData.length;i++){
//             //     if (groupedData[i].id === itemDivisions.divisions.id){
//             //         itemDivisions.divisions.works.push(groupedData[i].works)
//             //     }
//             // }
//
//
//             // console.log(groupedData)
//              groupedType[dataItemIdxUp].divisions.push(itemDivisions.divisions[0]);
//
//         }
//
//     });
...
//     return { divisions: groupedData, updivisions: groupedType };
//
//
// }
...
//
//     data.d.results.forEach((dataItem) => {
//         //console.log('dataItem.fldUpDivision',dataItem.fldUpDivision)
//         const dataItemIdx = groupedType.findIndex((grpItem) =>  grpItem.id === dataItem.fldUpDivisionId);
//         if (dataItemIdx === -1) {
//             const groupItem = parseDivisionItem(dataItem);
//             //console.log('groupItem',groupItem)
//             groupedType.push(groupItem);
...
//
//         else {
//             const itemDivision = parseDivisionItem(dataItem);
//             groupedData[dataItemIdx].divisions.push(itemDivision);
//
//         }
//
//     });
...
function groupByDivisionId(data) {
    ...
    console.log('divisions', arrayUpDivisions);
...
function groupBy(data, groupByField, optionLabelField, optionValueField) {
    const groupedData = [];
    dataObj.d.results.forEach((dataItem) => {
...
export const getData = () => fetch(`${BASE_URL}_api/web/lists(guid'${WORKS_LIST_GUID}')/items?$Select=Id,fldDivisionId,fldUpDivisionId,fldUpDivision/Id,fldUpDivision/Title,fldDivision/Id,fldDivision/Title,Title,fldUnits/Title,fldMTP,fldNote&$expand=fldDivision,fldUpDivision,fldUnits`,
    ...
    .catch(() => {
        return groupByDivisionId(dataObj);
    });
...
export const getItem = () => {
    ...
    const obj = {
        "updivisions": [{
            "id": 3,
            "title": "Внутренняя отделка помещений",
            "divisions": [{"id": 2, "title": "Отопление", "works": [{"id": "4", "title": "Установка радиаторов: с установкой кранов воздушных (воздухоотводчиков) и кранов проходных"}]}]
        }, {
            "id": 1,
            "title": "Кровельные работы",
            "divisions": [{
                "id": 1,
                "title": "Демонтажные работы",
                "works": [{"id": "1", "title": "Демонтаж электроразеток"}]
            }]
        }]
    }
    return fetch(url,
